...
function parseColor(color: any, defaultColor: number[] = [1, 0, 0]): number[] {
  ...
  if (typeof color === 'object') {
    ...
    const values = Object.values(color).filter((v) => typeof v === 'number') as number[];
    if (values.length >= 3) {
...
export default function FloatingAnimation({
  // Use vibrant colors from Framer default, but slightly tweaked to fit azrops
  ...
  useEffect(() => {
    ...
    // Style canvas to cover container completely
